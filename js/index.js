...
const drawCircle = (e) => {
  ctx.beginPath();
  // Pifagor teoremasi orqali radiusni hisoblash
  const radius = Math.sqrt(
      Math.pow(prevMouseX - e.offsetX, 2) + Math.pow(prevMouseY - e.offsetY, 2)
  );
  // Doira chizish
  ctx.arc(prevMouseX, prevMouseY, radius, 0, 2 * Math.PI);
  // To'ldirilgan yoki faqat chegarasi chizilgan holatni tanlash
...
const drawing = e => {
	...
	switch (selectedTool) {
		case 'brush':
			ctx.lineTo(e.offsetX, e.offsetY)
			ctx.stroke()
      canvas.classList.add("canvas-brush")
			break
		case 'rectangle':
			drawRectangle(e)
			break
		...
		case 'eraser':
			ctx.strokeStyle = '#fff'
      canvas.classList.add("canvas-eraser")
			ctx.lineTo(e.offsetX, e.offsetY)
			ctx.stroke()
			break
		default:
...
saveImageBtn.addEventListener('click', () => {
	...
	link.download = `zoyidjon-paint${Date.now()}.jpg`
...
const stopDraw = () => {
  ...
  canvas.classList.remove("canvas-brush")
}
